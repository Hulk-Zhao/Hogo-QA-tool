/** p3-print-measure2.mjs —— 更接近真实浏览器的打印测量：清除视口覆写后再 printToPDF。 */
...
async function main() {
  const profile = path.join(os.tmpdir(), 'hogo-p3print2-' + Date.now());
  fs.mkdirSync(profile, { recursive: true });
  const args = ['--headless=new','--disable-gpu','--no-first-run','--no-default-browser-check','--no-proxy-server','--remote-allow-origins=*','--remote-debugging-port=' + CDP_PORT,'--user-data-dir=' + profile];
  if (wide) args.push('--window-size=1600,1000');
  args.push('about:blank');
  const chrome = spawn(CHROME, args, { stdio: 'ignore' });
  const report = { steps: [], checks: {}, wide };
  let preview;
  try {
    ...
    report.checks.viewportW = await cdp.eval('window.innerWidth');
    ...
    report.checks.canvasInline = await cdp.eval('(function(){ var cv=document.querySelector(".print-chart canvas"); return cv? cv.getAttribute("style") : null; })()');
    report.checks.dpr = await cdp.eval('window.devicePixelRatio');

    // 关键：清除视口覆写，让打印按纸张重新布局（模拟真实 Chrome 打印）
    await cdp.send('Emulation.clearDeviceMetricsOverride');
    await sleep(800);
    report.checks.viewportWAfterClear = await cdp.eval('window.innerWidth');
    const pdf = await cdp.send('Page.printToPDF', { printBackground: true, preferCSSPageSize: true });
    fs.writeFileSync(ROOT + '/.probe/p3-report2.pdf', Buffer.from(pdf.data, 'base64'));
    report.checks.pdfBytes = Buffer.from(pdf.data, 'base64').length;
    // 打印态下 canvas 的 CSS 宽度（用 print 媒体查询量）
    await cdp.send('Emulation.setEmulatedMedia', { media: 'print' });
    await sleep(600);
    report.checks.printMediaCanvas = await cdp.eval('(function(){ var cv=document.querySelector(".print-chart canvas"); if(!cv) return null; var r=cv.getBoundingClientRect(); return { cssW: Math.round(r.width), cssH: Math.round(r.height), attrW: cv.width, attrH: cv.height, styleW: cv.style.width, maxW: getComputedStyle(cv).maxWidth }; })()');
    ...
    fs.writeFileSync(ROOT + '/.probe/p3-print-measure2' + (wide ? '-wide' : '') + '.json', JSON.stringify(report, null, 2), 'utf8');
