...
const APP_URL = 'http://127.0.0.1:8797/';
const CDP_PORT = 9477;
...
const SCENARIOS = {
  "asis": "return true;",
  "nobreak": "var e=document.querySelector(\".print-page-break\"); if(e) e.classList.remove(\"print-page-break\"); return true;",
  "block": "var e=document.querySelector(\"[data-testid=\\\"report-charts\\\"]\"); if(e){ e.style.display=\"block\"; } return true;",
  "flex": "var e=document.querySelector(\"[data-testid=\\\"report-charts\\\"]\"); if(e){ e.style.display=\"flex\"; } return true;",
  "avoidauto": "var e=document.querySelector(\"[data-testid=\\\"report-charts\\\"]\"); if(e){ [].slice.call(e.children).forEach(function(k){ k.style.breakInside=\"auto\"; }); } return true;",
  "maxh": "var e=document.querySelector(\"[data-testid=\\\"report-charts\\\"]\"); if(e){ [].slice.call(e.querySelectorAll(\"canvas.print-chart-snapshot\")).forEach(function(k){ k.style.maxHeight=\"190mm\"; k.style.objectFit=\"contain\"; }); } return true;",
  "spacer": "var e=document.querySelector(\"[data-testid=\\\"report-charts\\\"]\"); var f=document.querySelector(\".print-page-break\"); if(f) f.classList.remove(\"print-page-break\"); if(e){ var d=document.createElement(\"div\"); d.className=\"print-page-break\"; e.parentNode.insertBefore(d,e); } return true;",
  "stackauto": "var e=document.querySelector(\"[data-testid=\\\"report-charts\\\"]\"); var f=document.querySelector(\".print-page-break\"); if(e&&e.contains(f)) f.classList.remove(\"print-page-break\"); if(e) e.style.breakBefore=\"auto\"; return true;"
};

async function main() {
  const scenId = process.argv[2] || 'asis';
  const apply = SCENARIOS[scenId];
  if (!apply) throw new Error('unknown scenario ' + scenId);
  const profile = path.join(os.tmpdir(), 'hogo-p4one-' + Date.now());
  ...
  const report = { scenId, steps: [] };
  let preview;
  try {
    preview = spawn(process.execPath, ['node_modules/vite/bin/vite.js', 'preview', '--outDir', 'dist-server', '--host', '127.0.0.1', '--port', '8797', '--strictPort'], { cwd: ROOT, stdio: 'ignore' });
    ...
    report.perPage = files.map((f) => Number((ink[path.join(outdir, f)] || {}).inkRatio ?? -1));
    report.ok = true;
  } catch (e) {
    ...
    console.log(scenId.padEnd(12) + ' pages=' + report.pages + ' ink=' + JSON.stringify(report.perPage) + (report.error ? ' ERROR ' + report.error : ''));
  }
}
