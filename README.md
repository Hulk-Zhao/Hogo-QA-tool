...
| 报表导出 | ✅ | 导出范围 7 项逐项勾选（4 张表 + 3 张图，勾选状态刷新后保持）、**打印 / PDF 同步光栅快照**（A4 下有效 **307 DPI**、图铺满可打印区 99.9%、每页都非空白）、Excel 4 个数据 sheet + 按需追加的「图表」sheet（内嵌当前图表 PNG）与「AI 分析」sheet |
| AI 助手 | ✅ | OpenAI 兼容 API、三态探测、数据主权边界（默认仅发摘要）、AI 全面诊断（五节结构，可导出 Markdown）、**AI 分析导出**（6 条分析方向 × 6 个报表模块，逐模块串行分析，方向按模块自动过滤，可导出 Markdown / Excel / Word(.docx)） |
