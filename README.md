...
想自己确认，可以直接跑仓库里的**真 key 探针**（只从环境变量读 key，不写进任何文件、不落日志）：
...
它会依次做三件事：① 列出服务端**真实可用**的模型名（与你填的对照，不一致会明确告警）；
② 预检一次 `chat/completions`（「关闭模型思考」开 / 关各打一次，把服务端原文打出来）；
③ 真机跑一遍「导入 → 侧栏进 AI 助手 → 问第几子组触发哪条判异规则」，断言回复点名了
**子组编号**与**判异规则 id**、且**没有**再出现「摘要未提供 / 数据不足」这类推诿话术，
并在模型引用图表时确认真的画出了图（`ink > 0.01`，不是空 div）。
