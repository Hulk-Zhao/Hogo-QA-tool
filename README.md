...
「设置」页的模型名必须与**服务端真实模型名**逐字一致。DeepSeek 官方通常是
`deepseek-chat` / `deepseek-reasoner`；`deepseek-flash` 之类**不是**官方模型名，写错会直接被
服务端拒掉（HTTP 400，气泡下方会显示服务端原文）。Base URL 填 `https://api.deepseek.com/v1`。
