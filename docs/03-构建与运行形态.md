...
> | **项目库持久化保存** | ✅ | ✅ |
>
> 实测（Chrome，2026-09-23）：`file://` 下 `indexedDB.open()` **成功**，离线包与 HTTP 形态
> 一样走 IndexedDB，刷新 / 重开后项目仍在（`.probe/p13-file-idb.mjs` 直接问浏览器，
> `.probe/p10-import-library.mjs` 走完「导入 → 刷新 → 恢复」）。
> 此前这里写的「Chrome 在 `file://` 下阻止 IndexedDB」是旧版 Chrome 的行为，已不成立；
> 它当时还带偏了 `src/data/storage/browserAdapters.ts` 的**同步**探测 `probeKeyValueStore()`
> （至今仍对 `file://` 直接返回 false）—— 但应用走的是**异步**真实探测
> `probeKeyValueStoreAsync()`（见 `src/data/repositories/handle.ts`），所以不受影响。
> 仍然需要 `start.bat` 的场景是**连本机 AI 服务**：`file://` 页面直连 `localhost`
> 会被浏览器跨源策略拦下（见「设置」页的针对性提示）。
>
